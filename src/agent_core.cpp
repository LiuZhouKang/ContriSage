      ...
      core_prometheus_server(config.prometheus_listening_address, core_container_manager)
{
  if (!std::filesystem::exists("/var/run/docker.pid"))
  {
    ...
    core_config.enable_container_manager = true;
  }
...
TRACKER::tracker_event_handler agent_core::create_tracker_event_handler(const handler_config_data& config)
{
  ...
  {
    // 传递 container_manager 引用
    if constexpr (std::is_same_v<TRACKER, files_tracker> || std::is_same_v<TRACKER, syscall_tracker>) 
    {
      printf("Creating CSV event printer with container manager reference\n");
      return std::make_shared<typename TRACKER::csv_event_printer>(core_container_manager);
    } 
    else 
    {
    ...
    const std::vector<handler_config_data>& handler_configs)
{
    ...
    if (core_config.enable_container_manager) {
        auto container_handler = 
            std::make_shared<container_manager::container_info_handler<typename TRACKER::event>>(
                core_container_manager);
...
// {
//   std::cout<<core_config.enable_container_manager<<std::endl;
//   if (core_config.enable_container_manager)
//   {
//     spdlog::info("start container manager...");
//     printf("start container manager...\n");
//     core_container_manager.init();
//     spdlog::info("Container manager initialized successfully");
//     printf("Container manager initialized successfully\n");
...
void agent_core::start_container_manager(void) {
  if (core_config.enable_container_manager) {
    try {
      ...
      // 确保进程追踪器已启用
      bool process_tracker_enabled = false;
      for (auto &t : core_config.enabled_trackers) {
        if (t.name == "process") {
          process_tracker_enabled = true;
          break;
        }
      }
      
      if (!process_tracker_enabled) {
        core_config.enabled_trackers.push_back(tracker_config_data{"process", {}});
        spdlog::info("Added process tracker for container monitoring");
      }
    } catch (const std::exception &e) {
      spdlog::error("Container manager initialization failed: {}", e.what());
      core_config.enable_container_manager = false;
    }
  }
}
